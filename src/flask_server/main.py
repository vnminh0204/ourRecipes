...
import io
import json
import flask_server.dynamoDB.controller as dynamodb
import flask_server.dynamoDB.tokenBlacklisting as token_blacklisting
from flask_server.lib.nutritionCalculator import calculate_score
import jwt
...
from flask_server.recipe_recommendation.utils import data_io, nutrition_funcs
from flask_server.recipe_recommendation.recommend_model import k_nearest_recipes
...
def recommend_meals():
    ...
    csv_string = data_io.read_text_s3(
        AWS_ACCESS_KEY_ID,
    ...
    remain_ratio, remain_nutrition = nutrition_funcs.remain_total_nutri_ratio(
        meal_request, meal_names, randomized_ratios
    )
    normalized_ratios = nutrition_funcs.re_normalize_ratio(
        randomized_ratios, remain_ratio
    )
    recipe_recommendations = k_nearest_recipes(
        df, meal_request, remain_nutrition, meal_names, normalized_ratios
    )
